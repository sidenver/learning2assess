"""Usage: train_ranking_resample.py [options]
...
--train=FILE         train_file [default: task_B.train.all.prediction.train.svm]
--test=FILE          test_file [default: task_B.train.all.prediction.train.svm]
"""
...
# def split_and_convert_to_libsvm_format(path, test_p=0.2):
#     out_path_train = path + '.train.svm'
#     out_path_dev = path + '.dev.svm'
#     with open(out_path_train, 'w') as outputs_train, open(out_path_dev, 'w') as outputs_dev, open(path, 'r') as inputs:
#         for line in inputs:
...
if __name__ == '__main__':
    ...
    _TRAIN_DATA_PATH = args['--train']
    _TEST_DATA_PATH = args['--test']
    ...
    with open(os.path.join(args['--output'], '{}_{}.json'.format(_LOSS, args['--seed'])), 'w') as outfile:
